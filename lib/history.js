...
History.prototype.getAvgTransactionRate = function ()
{
	var txCount = _(this._items)
		.sortByOrder('height', false)
		.slice(0, MAX_BINS)
		...
		.value();
		//console.log("txCount:", _.sum(txCount), "Num of values:", txCount.length);
	var blocktimeHistory = _(this._items)
		...
		.value();
		//console.log("blocktimeHistory", _.sum(blocktimeHistory), "Num of values:", blocktimeHistory.length);
		//console.log("Average TX/S", _.sum(txCount) / ((_.sum(blocktimeHistory)) / 1000));
		return _.sum(txCount) / ((_.sum(blocktimeHistory)) / 1000);
}
...
History.prototype.getCharts = function()
{
	if(this._callback !== null)
	{
		...
		this._callback(null, {
			...
			avgTransactionRate: this.getAvgTransactionRate()
		});
	}
}
